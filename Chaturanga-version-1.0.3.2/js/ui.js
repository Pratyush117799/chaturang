...
document.addEventListener('DOMContentLoaded', () => {
  try {
    ...
    // Helper: get the move from the right bot level for the current turn
    function getBotMove() {
      ...
      return window.ChaturangaTieredBot
        ? window.ChaturangaTieredBot.getMove(game, elo)
        : null;
    }
    ...
    if (boardSizeEl && boardSizeLabel) {
      ...
      // Base square size is 70px on desktop, dynamic on mobile.
      const getBaseSize = () => window.innerWidth < 640 ? Math.floor((window.innerWidth - 40) / 8) : 70;
      const newSize = Math.floor(getBaseSize() * (val / 100));
      ...
      boardSizeEl.addEventListener('input', () => {
        ...
        const s = Math.floor(getBaseSize() * (v / 100));
