...
(function (global) {
  ...
  function initClockUI(anchorId, modeKey, playerCount) {
    console.log('ChaturangaClockUI: initClockUI starting... anchorId:', anchorId, 'modeKey:', modeKey);
    const n    = playerCount || 4;
    const mode = (typeof ChaturangaClock !== 'undefined' && ChaturangaClock.MODES) 
        ? (ChaturangaClock.MODES[modeKey] || ChaturangaClock.MODES.chirantan)
        : { name: 'Fallback', initial: null };
    console.log('ChaturangaClockUI: mode identified:', mode.name);
    ...
    for (let i = 0; i < n; i++) {
      ...
      time.textContent = (typeof ChaturangaClock !== 'undefined') ? ChaturangaClock.formatTime(mode.initial) : '--:--';
    ...
    // ── Inject into DOM ──
    console.log('ChaturangaClockUI: injecting into DOM...');
    const anchor = document.getElementById(anchorId);
    console.log('ChaturangaClockUI: anchor found:', !!anchor);
    if (anchor && anchor.parentNode) {
      console.log('ChaturangaClockUI: inserting before anchor:', anchor.parentNode.tagName);
      anchor.parentNode.insertBefore(strip, anchor);
    } else {
      console.log('ChaturangaClockUI: no anchor parent, prepending to body');
