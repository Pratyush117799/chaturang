...
let SEARCH_DEADLINE = Infinity; // wall-clock cutoff, in ms since epoch
...
(function () {
  ...
  function teamEval(game, playerId) {
    ...
    // 5. Mobility & Activity estimation (fast, no getLegalMoves loop)
    ...
    for (const p of game.players) {
      ...
      for (let r = 1; r <= 8; r++) {
        for (let f = 0; f < 8; f++) {
          ...
          if (pc && pc.owner === p.id) {
            mob += (pc.type === 'horse' ? 4 : pc.type === 'rook' ? 6 : pc.type === 'elephant' ? 2 : pc.type === 'pawn' ? 1 : 2);
          }
        }
      }
    ...
    // 6. King safety: penalty for exposed king advancing off back rank
    for (let r = 1; r <= 8; r++) {
      for (let f = 0; f < 8; f++) {
        const sq    = String.fromCharCode(97 + f) + r;
        const piece = game.board.get(sq);
        if (!piece || piece.type !== 'king') continue;
        const isAllyKing = isAlly(piece.owner);
        const kingAdv = getPawnAdvancement(sq, piece.owner);
        if (isAllyKing) {
          if (kingAdv > 1) score -= (kingAdv - 1) * 3.0 * egF;
        } else {
          if (kingAdv > 1) score += (kingAdv - 1) * 1.5 * egF;
        }
      }
    }
  ...
  function expectiTeamAB(game, depth, alpha, beta, playerId, useQuiesc, tt) {
     ...
     if (Date.now() > SEARCH_DEADLINE) return teamEval(game, playerId);
  ...
  function grandmasterGetMove(game) {
    ...
    for (const move of orderMoves(game, moves)) {
      ...
      const score = expectiTeamAB(game, 2, -Infinity, Infinity, playerId, false, null);
  ...
  function maharajaGetMove(game) {
    ...
    for (const move of orderMoves(game, moves)) {
      ...
      const score = expectiTeamAB(game, 3, -Infinity, Infinity, playerId, true, null);
