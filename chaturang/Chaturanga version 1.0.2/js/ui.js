// Chaturanga 1.0.2 - UI with four dice, auto-roll, piece images, persistence
document.addEventListener('DOMContentLoaded', () => {
  try {
    ...
    function tryAutoRoll() {
      if (!autoRollDiceEl || !autoRollDiceEl.checked || game.gameOver) return;
      const player = game.getPlayer();
      if (!player || player.eliminated) return;
      setTimeout(() => {
        if (game.forcedPiece != null) return;
    ...
    function downloadHistory() {
      ...
      a.download = 'chaturanga_' + new Date().toISOString().slice(0, 19).replace(/[:T]/g, '-') + (suffix ? '_' + suffix : '') + '.txt';
