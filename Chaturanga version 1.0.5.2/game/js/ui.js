  ...
  const playMoveSfx = () => playTone(520, 'sine', 0.12, 0.15);
    ...
    function performRender() {
      ...
      try {
        ...
        console.error('Chaturanga: performRender ERROR:', e);
    ...
    // ── Settings Drawer ────────────────────────────────────────────────────
    if (settingsToggle && !settingsToggle._safeWired) {
      settingsToggle.addEventListener('click', () => settingsDrawer && settingsDrawer.classList.toggle('open'));
      settingsToggle._safeWired = true;
    }
    if (settingsClose && !settingsClose._safeWired) {
      settingsClose.addEventListener('click', () => settingsDrawer && settingsDrawer.classList.remove('open'));
      settingsClose._safeWired = true;
    }
    ...
    if (boardSizeEl && boardSizeLabel) {
      ...
      const saved = safeLS.getItem('chaturanga_boardSize_v3');
      ...
      boardSizeEl.addEventListener('input', () => {
        ...
        applySize(v);
        safeLS.setItem('chaturanga_boardSize_v3', String(v));
      });
    ...
    // ── Board Theme ────────────────────────────────────────────────────────
    if (boardThemeEl) {
      const savedTheme = safeLS.getItem('chaturanga_boardTheme_v3') || 'standard';
      ...
      boardThemeEl.addEventListener('change', () => {
        ...
        safeLS.setItem('chaturanga_boardTheme_v3', theme);
      });
    }
    ...
    if (pieceStyleEl) {
      const savedStyle = safeLS.getItem('chaturanga_pieceStyle_v4') || 'traditional';
      pieceStyleEl.value = savedStyle;
      pieceStyleEl.addEventListener('change', () => {
        safeLS.setItem('chaturanga_pieceStyle_v4', pieceStyleEl.value);
    ...
    function downloadHistory() {
      ...
      const rounds = [];
      for (let i = 0; i < game.moveHistory.length; i += 4) rounds.push(game.moveHistory.slice(i, i + 4));
      let content = 'Chaturanga v1.0.5 — Game Log\n';
      content += '================================\n\n';
